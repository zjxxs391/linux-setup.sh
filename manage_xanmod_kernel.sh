...
install_xanmod() {
    ...
    # 检查 CPU 支持的指令集级别
    local check_script_url="${YES_CN}https://raw.githubusercontent.com/SuperNG6/linux-setup.sh/main/check_x86-64_psabi.sh"
    echo "INFO: 正在从 $check_script_url 下载并检查CPU支持级别..."
    local cpu_support_info
    cpu_support_info=$(/usr/bin/awk -f <(wget -qO - "$check_script_url"))
